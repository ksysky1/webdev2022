...
public class Process1Drag : MonoBehaviour
{
    ...
    void Update()
    {
       if(flag == -1){
            if(ShortDist == Dist0){
                Vector3 destination = Box0.transform.position + new Vector3(125,30,10); //box0내의 정해진 위치로 이동하게 하기.
                transform.position = Vector3.MoveTowards(transform.position, destination, 1);
            }
            else if(ShortDist == Dist1){
                Vector3 destination = Box1.transform.position + new Vector3(-65,30,-10); //box1내의 정해진 위치로 이동하게 하기.
                transform.position = Vector3.MoveTowards(transform.position, destination, 1);
            }
            else if(ShortDist == Dist2){
                Vector3 destination = Box2.transform.position + new Vector3(45,30,-10); //box2내의 정해진 위치로 이동하게 하기.
                transform.position = Vector3.MoveTowards(transform.position, destination, 1);
            }
            else if(ShortDist == Dist3){
                Vector3 destination = Box3.transform.position + new Vector3(-65,30,-10); //box3내의 정해진 위치로 이동하게 하기.
                transform.position = Vector3.MoveTowards(transform.position, destination, 1);
            }
       }


        if(transform.position == Box1.transform.position + new Vector3(-65,30,-10)){
            flag = 0; // 입력 박스 안에 들어간 적 있음 표시
        }

        else if(transform.position == Box3.transform.position + new Vector3(-65, 30, -10)){
            flag = 2; // 출력 박스 안에 들어간 적 있음 표시
        }


        if(flag == 0 || flag == 2){// 답이 틀렸을 때(처리 글자가 입력박스나 출력박스로 들어갔을 때)
            Vector3 ReturnDest = Box0.transform.position + new Vector3(125, 30, 10); //박스0내의 정해진 위치로 보내기
